...
"""
登录文件
采用https://github.com/egrcc/zhihu-python/blob/master/auth.py的实现
增加headers验证
暂时取代login.py v1.0的实现,因为会出现报告登录频繁的错误,待解决.
"""
...
def download_captcha():
    ...
    r = session.get(url, params={"r": int(time.time()), "type": "login"}, verify=False, headers=headers)
...
def login(account=None, password=None, debug=False):
    if islogin() == True:
